...
5. En Cloudflare, añade como secretos del Worker `SUPABASE_SERVICE_ROLE_KEY`, `VAPID_PRIVATE_KEY` y `VAPID_SUBJECT`. `VAPID_SUBJECT` debe ser una URL o un correo con prefijo `mailto:`, por ejemplo `mailto:tu-correo@example.com`. Añade también `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `VAPID_PUBLIC_KEY` y `ALLOWED_ORIGIN` (la URL exacta de la aplicación de Cloudflare Pages). La campana guarda y borra suscripciones con la sesión normal del usuario; la service role queda para los avisos programados y para enviar avisos a otros miembros.
